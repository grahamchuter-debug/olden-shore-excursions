 ...
 */
/** Phase O-2: live code flag stays OFF until Graham unlock (O-3+). */
...
export function liveCheckoutBlock(
  ...
  if (!LIVE_PAYMENTS_CODE_ENABLED) {
    return {
      ...
      message: "Live Stripe is not enabled for Olden. The live code flag is off. No payment was taken.",
    };
  }
  ...
  if (!bookingsAreEnabled(env)) {
    return {
      ...
      message: "Olden online booking is switched off. No payment was taken.",
    };
  }
  if (!env.DB) {
    return {
  ...
  if (!site.includes("oldenshoreexcursions.com")) {
    return {
      code: "SITE_BASE_URL",
      message: "Live Checkout requires SITE_BASE_URL on oldenshoreexcursions.com. No payment was taken.",
    };
  }
  if (!product) {
    return {
